...
last_index = (
    next(index for index, line in enumerate(all_lines[start_index:]) if "===" in line) + start_index
)
...
data_frame = pd.DataFrame(
    dict(test_time=timing_column, test_name=short_name, long_name=long_name, type=type)
)
...
data_frame_to_display = data_frame[
    ["test_name", "type", "test_time", "%of_total_time", "%cum_total_time", "long_name"]
]
