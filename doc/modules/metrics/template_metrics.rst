...
* "recovery_slope": speed to recover from the negative peak to 0 in :math:`\mu V / s`
* "repolarization_slope": speed to repolarize from the positive peak to 0 in :math:`\mu V / s`
...
* "velocity_above": the velocity in :math:`\mu m/ms` above the max channel of the template
* "velocity_below": the velocity in :math:`\mu m/ms` below the max channel of the template
* "exp_decay": the exponential decay in :math:`1 / \mu m` of the template amplitude over distance
