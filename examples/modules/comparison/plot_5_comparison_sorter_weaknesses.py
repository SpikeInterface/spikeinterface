...
print('well_detected', comp.get_well_detected_units(accuracy=0.75))


##############################################################################
# Here we can explore **"false positive units"** units that do not exists in ground truth

print('false_positive', comp.get_false_positive_units(bad_redundant_threshold=0.2))

##############################################################################
# Here we can explore **"redundant units"** units that do not exists in ground truth

print('redundant', comp.get_redundant_units(bad_redundant_threshold=0.2))


##############################################################################
# Here we can explore **"bad units"** units that do not exists in ground truth
