...
def _create_recording():
    ...
    rec, sorting = si.toy_example(
        num_segments=1, duration=200, seed=1, num_channels=16, num_columns=2
    )
...
def open_sigui():
    ...
    pc = si.compute_principal_components(
        we, n_components=3, mode="by_channel_local", whiten=True, dtype="float32"
    )
...
def _clean():
    ...
    folders = [
        ...
        "tridesclous_output",
        "tridesclous_waveforms",
        "spykingcircus_output",
        "spykingcircus_waveforms",
        "phy_example",
    ]
