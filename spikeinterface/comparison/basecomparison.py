...
from .comparisontools import make_best_match, make_hungarian_match, make_possible_match
...
class BaseMultiComparison(BaseComparison):
    ...
    def _do_agreement(self):
        ...
        # save new units
        self.subgraphs = [
            self.clean_graph.subgraph(c).copy() for c in nx.connected_components(self.clean_graph)
        ]
...
class BasePairComparison(BaseComparison):
    ...
    def __init__(
        self, object1, object2, name1, name2, match_score=0.5, chance_score=0.1, verbose=False
    ):
        BaseComparison.__init__(
            self,
...
class MixinSpikeTrainComparison:
    ...
    def set_frames_and_frequency(self, sorting_list):
        ...
        # check num segments
        if not np.all(
            sorting.get_num_segments() == sorting0.get_num_segments() for sorting in sorting_list
        ):
        ...
        sampling_freqs = np.array(
            [sorting.get_sampling_frequency() for sorting in sorting_list], dtype="float64"
        )
