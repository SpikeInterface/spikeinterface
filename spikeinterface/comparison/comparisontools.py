...
def make_match_count_matrix(sorting1, sorting2, delta_frames, n_jobs=1):
    ...
    for segment_index in range(sorting1.get_num_segments()):
        s2_spiketrains = [
            sorting2.get_unit_spike_train(u2, segment_index=segment_index) for u2 in unit2_ids
        ]

        match_event_count_segment = Parallel(n_jobs=n_jobs)(
            delayed(count_match_spikes)(
                sorting1.get_unit_spike_train(u1, segment_index=segment_index),
                s2_spiketrains,
                delta_frames,
            )
...
def make_agreement_scores_from_count(match_event_count, event_counts1, event_counts2):
    ...
    agreement_scores = pd.DataFrame(
        agreement_scores, index=match_event_count.index, columns=match_event_count.columns
    )
    return agreement_scores
...
def do_score_labels(sorting1, sorting2, delta_frames, unit_map12, label_misclassification=False):
    ...
    sts1 = {
        u1: [sorting1.get_unit_spike_train(u1, seg_index) for seg_index in range(num_segments)]
        for u1 in unit1_ids
    }
    sts2 = {
        u2: [sorting2.get_unit_spike_train(u2, seg_index) for seg_index in range(num_segments)]
        for u2 in unit2_ids
    }
