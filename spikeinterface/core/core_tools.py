            ...
            if len(v) > 0:
                if isinstance(v[0], dict):
                    ...
                    if v_arr.dtype.kind not in ("b", "i", "u", "f", "S", "U", "O"):
                        print(
                            f"Skipping field {k}: only int, uint, bool, float, or str types can be serialized"
                        )
                        continue
    ...
    **job_kwargs,
    ...
    for segment_index in range(recording.get_num_segments()):
        ...
        rec_memmaps_dict.append(
            dict(filename=str(file_path), dtype=dtype, mode="r+", offset=byte_offset, shape=shape)
        )
    ...
    executor = ChunkRecordingExecutor(
        ...
        init_args,
        verbose=verbose,
        job_name="write_binary_recording",
        **job_kwargs,
    )
    executor.run()
...
def write_memory_recording(recording, dtype=None, verbose=False, auto_cast_uint=True, **job_kwargs):
    ...
    executor = ChunkRecordingExecutor(
        ...
        init_args,
        verbose=verbose,
        job_name="write_memory_recording",
        **job_kwargs,
    )
    executor.run()
