...
from spikeinterface.core.core_tools import check_json, define_function_from_class
...
class InjectTemplatesRecordingSegment(BaseRecordingSegment):
    def __init__(
        ...
        self.num_samples = (
            parent_recording_segment.get_num_frames() if num_samples is None else num_samples
        )
    ...
    def get_traces(
        ...
        channel_indices = (
            list(range(self.templates.shape[2])) if channel_indices is None else channel_indices
        )
        ...
        start = np.searchsorted(
            self.spike_vector["sample_ind"], start_frame - self.templates.shape[1], side="left"
        )
        end = np.searchsorted(
            self.spike_vector["sample_ind"], end_frame + self.templates.shape[1], side="right"
        )
