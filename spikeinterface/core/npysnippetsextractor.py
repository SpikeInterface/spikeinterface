from .basesnippets import BaseSnippets, BaseSnippetsgSegment
...
class NpySnippetsExtractor(BaseSnippets):
    ...
    def __init__(self, file_path):
        ...
        num_segment = int(npy['num_segment'][0])
        nbefore = (npy['nbefore'][0])
        if nbefore<0: nbefore=None
        sampling_frequency = float(npy['sampling_frequency'][0])

        BaseSnippets.__init__(self, sampling_frequency, unit_ids)
