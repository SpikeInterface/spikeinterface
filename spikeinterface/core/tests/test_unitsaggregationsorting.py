...
from spikeinterface.core import NpzSortingExtractor, aggregate_units, create_sorting_npz
...
def test_unitsaggregationsorting():
    ...
    for seg in range(num_seg):
        ...
        assert np.allclose(
            spiketrain1_1, sorting_agg.get_unit_spike_train(unit_ids[1], segment_index=seg)
        )
        assert np.allclose(
            spiketrains2_0,
