    ...
    for mode in ["folder", "memory"]:
        for sparsity in [None, sparsity_ext]:
            ...
            we = WaveformExtractor.create(
                recording, sorting, wf_folder, mode=mode, sparsity=sparsity
            )
            we.set_params(ms_before=1.0, ms_after=1.6, max_spikes_per_unit=500)
            ...
            for unit_id in we_saved.unit_ids:
                ...
                assert np.array_equal(
                    we.get_sampled_indices(unit_id), we_saved.get_sampled_indices(unit_id)
                )
            ...
            for unit_id in we_saved_zarr.unit_ids:
                ...
                assert np.array_equal(
                    we.get_sampled_indices(unit_id), we_saved_zarr.get_sampled_indices(unit_id)
                )
...
def test_extract_waveforms():
    ...
    with pytest.raises(Exception):
        we1 = extract_waveforms(
            recording, sorting, folder_crash, max_spikes_per_unit=None, return_scaled=False
        )
...
def test_recordingless():
    ...
    assert np.array_equal(
        we.recording.get_channel_locations(), np.array(we_loaded.get_channel_locations())
    )
...
def test_compute_sparsity():
    ...
    for kwargs in [
        dict(method="radius", radius_um=50.0),
        dict(method="best_channels", num_channels=2),
    ]:
