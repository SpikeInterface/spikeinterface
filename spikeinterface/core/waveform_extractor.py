...
from .job_tools import _shared_job_kwargs_doc, fix_job_kwargs, split_job_kwargs
from .recording_tools import check_probe_do_not_overlap
from .sparsity import ChannelSparsity, _sparsity_doc, compute_sparsity
...
class WaveformExtractor:
    ...
    def __init__(
        ...
        if recording is None:
            ...
            assert (
                recording.get_num_segments() == sorting.get_num_segments()
            ), "The recording and sorting objects must have the same number of segments!"
            np.testing.assert_almost_equal(
                recording.get_sampling_frequency(), sorting.get_sampling_frequency(), decimal=2
            )
    ...
    def set_params(
        self, ms_before=1.0, ms_after=2.0, max_spikes_per_unit=500, return_scaled=False, dtype=None
    ):
        """
    ...
    def select_units(self, unit_ids, new_folder=None, use_relative_path=False):
        ...
        if self.folder is not None and new_folder is not None:
            if self.format == "binary":
                ...
                waveforms_files = [
                    f for f in (self.folder / "waveforms").iterdir() if f.suffix == ".npy"
                ]
                ...
                template_files = [
                    f for f in self.folder.iterdir() if "template" in f.name and f.suffix == ".npy"
                ]
            ...
            if self.has_waveforms():
                ...
                for unit_id in unit_ids:
                    we._memory_objects["wfs_arrays"][unit_id] = self._memory_objects["wfs_arrays"][
                        unit_id
                    ]
                    we._memory_objects["sampled_indices"][unit_id] = self._memory_objects[
                        "sampled_indices"
                    ][unit_id]
            ...
            if self.has_waveforms():
                ...
                for unit_ind, unit_id in enumerate(self.unit_ids):
                    ...
                    waveform_group.create_dataset(
                        name=f"waveforms_{unit_id}", data=waveforms, compressor=compressor
                    )
                    waveform_group.create_dataset(
    ...
    def run_extract_waveforms(self, seed=None, **job_kwargs):
        ...
        for segment_index in range(self.sorting.get_num_segments()):
            num_in_seg = np.sum(
                [selected_spikes[unit_id][segment_index].size for unit_id in unit_ids],
                dtype=np.int64,
            )
...
def select_random_spikes_uniformly(
    ...
    for unit_id in unit_ids:
        # spike per segment
        n_per_segment = [
            sorting.get_unit_spike_train(unit_id, segment_index=i).size for i in range(num_seg)
        ]
...
def extract_waveforms(
    ...
    **kwargs,
    ...
    if load_if_exists is None:
        ...
        warn(
            "load_if_exists=True/false is deprcated. Use load_waveforms() instead.",
            DeprecationWarning,
            stacklevel=2,
        )
