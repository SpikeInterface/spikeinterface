...
from ..qualitymetrics import compute_firing_rates, compute_refrac_period_violations
...
def get_potential_auto_merge(
    ...
    if "check_increase_score" in steps:
        pair_mask = check_improve_contaminations_score(
            we,
            pair_mask,
            contaminations,
...
def compute_correlogram_diff(
    ...
    for unit_ind1 in range(n):
        for unit_ind2 in range(unit_ind1 + 1, n):
            ...
            win_size = int(
                round((num1 * win_sizes[unit_ind1] + num2 * win_sizes[unit_ind2]) / (num1 + num2))
            )
            ...
            diff1 = np.sum(np.abs(cross_corr[corr_inds - shift] - auto_corr1[corr_inds])) / len(
                corr_inds
            )
            diff2 = np.sum(np.abs(cross_corr[corr_inds - shift] - auto_corr2[corr_inds])) / len(
                corr_inds
            )
