...
class RemoveDuplicatedSpikesSortingSegment(BaseSortingSegment):
    ...
    def __init__(self, parent_segment: BaseSortingSegment, censored_period: int, unit_ids, method: str) -> None:
        ...
        for unit_id in unit_ids:
            self._duplicated_spikes[unit_id] = find_duplicated_spikes(parent_segment.get_unit_spike_train(unit_id, start_frame=None, end_frame=None), censored_period, method=method)
...
def _find_duplicated_spikes_random(spike_train: np.ndarray, censored_period: int, seed: Optional[int] = 186472189) -> np.ndarray:
    """
...
if HAVE_NUMBA:
    ...
    def _find_duplicated_spikes_keep_first(spike_train, censored_period):
    ...
    @numba.jit((numba.int64[::1], numba.int32), nopython=True, nogil=True, cache=True, parallel=True)
    def _find_duplicated_spikes_keep_last(spike_train, censored_period):
        indices_of_duplicates = numba.typed.List()
        N = len(spike_train)
...
def find_duplicated_spikes(spike_train, censored_period, method: str = "random") -> np.ndarray:
    """
    ...
    if method == "random":
        return _find_duplicated_spikes_random(spike_train.astype(np.int64), censored_period)
    elif method == "keep_first":
        assert HAVE_NUMBA
        return _find_duplicated_spikes_keep_first(spike_train.astype(np.int64), censored_period)
    elif method == "keep_last":
        assert HAVE_NUMBA
        return _find_duplicated_spikes_keep_last(spike_train.astype(np.int64), censored_period)
    else:
