...
from spikeinterface.curation import find_duplicated_spikes, remove_duplicated_spikes
    ...
    for method in (
        ...
        for segment_index in range(sorting.get_num_segments()):
            for unit_id in sorting.unit_ids:
                ...
                assert np.all(
                    np.diff(new_sorting.get_unit_spike_train(unit_id, segment_index=segment_index))
                    > censored_period
                )
...
def test_find_duplicated_spikes() -> None:
    ...
    assert (
        len(find_duplicated_spikes(spike_train, censored_period=5, method="random", seed=42)) >= 3
    )
    ...
    assert (
        len(find_duplicated_spikes(spike_train, censored_period=5, method="keep_first_iterative"))
        == 3
    )
    assert (
        len(find_duplicated_spikes(spike_train, censored_period=5, method="keep_last_iterative"))
        == 3
    )
