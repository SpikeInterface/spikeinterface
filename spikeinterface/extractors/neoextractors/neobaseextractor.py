...
class NeoBaseSortingExtractor(_NeoBaseExtractor, BaseSorting):
    ...
    def __init__(
        self, block_index=None, sampling_frequency=None, use_natural_unit_ids=False, **neo_kwargs
    ):
        _NeoBaseExtractor.__init__(self, block_index, **neo_kwargs)
        ...
        if sampling_frequency is None:
            ...
            stream_index = np.where(self.neo_reader.header["signal_streams"]["id"] == stream_id)[0][
                0
            ]
        else:
            stream_index = None
...
class NeoSortingSegment(BaseSortingSegment):
    ...
    def get_unit_spike_train(self, unit_id, start_frame, end_frame):
        ...
        if self._t_start is None:
            ...
            spike_frames = ((spike_times - self._t_start) * self._sampling_frequency).astype(
                "int64"
            )
