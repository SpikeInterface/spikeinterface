...
class OpenEphysLegacyRecordingExtractor(NeoBaseRecordingExtractor):
    ...
    def __init__(
        self, folder_path, stream_id=None, stream_name=None, block_index=None, all_annotations=False
    ):
...
class OpenEphysBinaryRecordingExtractor(NeoBaseRecordingExtractor):
    ...
    def __init__(
        ...
        if "NI-DAQmx" not in stream_name:
            settings_file = self.neo_reader.folder_structure[record_node]["experiments"][exp_id][
                "settings_file"
            ]
        ...
        if load_sync_timestamps:
            ...
            for segment_index in range(self.get_num_segments()):
                ...
                try:
                    self.set_times(
                        times=sync_times, segment_index=segment_index, with_warning=False
                    )
