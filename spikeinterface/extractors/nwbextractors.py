    ...
    def __init__(self, file_path: PathType, electrical_series_name: str = None, load_time_vector: bool = False,
        ...
        with NWBHDF5IO(self._file_path, mode='r',load_namespaces=True) as io:
            ...
            self._es = get_electrical_series(
                nwbfile, self._electrical_series_name)
            ...
            if load_time_vector and timestamps is not None:
                times_kwargs = dict(time_vector=np.array(self._es.timestamps))
            else:
        ...
        recording_segment = NwbRecordingSegment(nwbfile_path=self._file_path,
                                                electrical_series_name=self._electrical_series_name,
                                                num_frames=num_frames, times_kwargs=times_kwargs)
        self.add_recording_segment(recording_segment)

class NwbRecordingSegment(BaseRecordingSegment):
    def __init__(self, nwbfile_path, electrical_series_name, num_frames, times_kwargs):
        BaseRecordingSegment.__init__(self, **times_kwargs)
        self._nwbfile_path = nwbfile_path
        self._electrical_series_name = electrical_series_name
    ...
    def get_traces(self, start_frame, end_frame, channel_indices):
        with NWBHDF5IO(self._nwbfile_path, 'r', load_namespaces=True) as io:
            nwbfile = io.read()
    ...
    def __init__(self, file_path: PathType, electrical_series_name: str = None, sampling_frequency: float = None,
        ...
        with NWBHDF5IO(self._file_path, mode='r', load_namespaces=True) as io:
            ...
            if sampling_frequency is None:
                ...
                self._es = get_electrical_series(nwbfile, self._electrical_series_name)
                # get rate
                timestamps = None
                if self._es.rate is not None:
                    ...
                    if hasattr(self._es, "timestamps"):
                        if self._es.timestamps is not None:
                            timestamps = np.array(self._es.timestamps)
                            sampling_frequency = 1 / np.median(np.diff(timestamps[samples_for_rate_estimation]))
            ...
            for column in list(nwbfile.units.colnames):
                if column == 'spike_times':
                    continue
                # if it is unit_property
                property_values = nwbfile.units[column][:]
        ...
        sorting_segment = NwbSortingSegment(nwbfile_path=self._file_path, sampling_frequency=sampling_frequency,
                                            timestamps=timestamps)
        self.add_sorting_segment(sorting_segment)


class NwbSortingSegment(BaseSortingSegment):
    def __init__(self, nwbfile_path, sampling_frequency, timestamps):
    ...
    def get_unit_spike_train(self,
        ...
        if end_frame is None:
            end_frame = np.inf
        with NWBHDF5IO(self._nwbfile_path, mode='r', load_namespaces=True) as io:
            nwbfile = io.read()
            times = nwbfile.units['spike_times'][list(nwbfile.units.id[:]).index(unit_id)][:]

            if self._timestamps is not None:
                frames = np.searchsorted(times, self._timestamps).astype('int64')
            else:
