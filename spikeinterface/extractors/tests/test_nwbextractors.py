...
class GainsAndOffsetTest(unittest.TestCase):
    def setUp(self):
        ...
        electrode_group = self.nwbfile.create_electrode_group(
            "electrode", device=device, location="brain", description="fake"
        )
    ...
    def test_offset_extraction_from_electrode_table(self):
        offset_data = self.rng.integers(low=0, high=20, size=self.number_of_electrodes).astype(
            "float"
        )
        self.nwbfile.add_electrode_column(
            name="offset", data=offset_data, description="Level offset"
        )
        number_of_electrodes_in_electrical_series = 5
        options = range(self.number_of_electrodes)
        region_indexes = sorted(
            self.rng.choice(
                options, size=number_of_electrodes_in_electrical_series, replace=False
            ).tolist()
        )
        electrode_region = self.nwbfile.create_electrode_table_region(
            region_indexes, "record electrodes"
        )
        ...
        electrical_series = ElectricalSeries(
            name=electrical_series_name, data=data, electrodes=electrode_region, rate=20_000.0
        )
        ...
        recording_extractor = NwbRecordingExtractor(
            self.nwbfile_path, electrical_series_name=electrical_series_name
        )
