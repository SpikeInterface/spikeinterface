...
class CorrelogramsCalculator(BaseWaveformExtractorExtension):
    ...
    def get_data(self):
        ...
        assert (
            self._extension_data["ccgs"] is not None and self._extension_data["bins"] is not None
        ), msg
...
def compute_crosscorrelogram_from_spiketrain(spike_times1, spike_times2, window_size, bin_size):
    ...
    """
    assert HAVE_NUMBA
    return _compute_crosscorr_numba(
        spike_times1.astype(np.int64), spike_times2.astype(np.int64), window_size, bin_size
    )
...
if HAVE_NUMBA:
    ...
    @numba.jit(
        (numba.int64[::1], numba.int64[::1], numba.int32, numba.int32),
        nopython=True,
        nogil=True,
        cache=True,
    )
