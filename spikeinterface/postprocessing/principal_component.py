...
class WaveformPrincipalComponent(BaseWaveformExtractorExtension):
    ...
    def _fit_by_channel_local(self, n_jobs, progress_bar):
        ...
        pca_models = [
            IncrementalPCA(n_components=p["n_components"], whiten=p["whiten"]) for _ in channel_ids
        ]
...
def compute_principal_components(
    ...
    **job_kwargs,
    ...
    if load_if_exists and waveform_extractor.is_extension(
        ...
        pc.set_params(
            n_components=n_components, mode=mode, whiten=whiten, dtype=dtype, sparsity=sparsity
        )
        pc.run(**job_kwargs)
