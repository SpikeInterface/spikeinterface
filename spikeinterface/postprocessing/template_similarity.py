...
def check_equal_template_with_distribution_overlap(
    ...
    for shift in range(num_shift * 2 + 1):
        ...
        scalar_product0 = np.sum(
            (wfs0 - template0_[np.newaxis, :, :]) * vector_0_1[np.newaxis, :, :], axis=(1, 2)
        )
        scalar_product1 = np.sum(
            (wfs1 - template0_[np.newaxis, :, :]) * vector_0_1[np.newaxis, :, :], axis=(1, 2)
        )
