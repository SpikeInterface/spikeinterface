...
from ..core.waveform_extractor import BaseWaveformExtractorExtension, WaveformExtractor
...
def make_initial_guess_and_bounds(wf_ptp, local_contact_locations, max_distance_um, initial_z=20):
    ...
    x0[2] = initial_z
    initial_alpha = (
        np.sqrt(np.sum((com - local_contact_locations[ind_max, :]) ** 2) + initial_z**2) * max_ptp
    )
    x0[3] = initial_alpha
...
def estimate_distance_error_with_log(vec, wf_ptp, local_contact_locations, maxptp):
    ...
    err = (
        np.square(wf_ptp / maxptp - ptp_at(x, y, z, alpha, local_contact_locations)).mean()
...
def make_radial_order_parents(geom, neighbours_mask, n_jumps_per_growth=1, n_jumps_parent=3):
    ...
    for channel, neighbors in enumerate(neighbours_mask):
        ...
        while len(already_seen) < (neighbors < n_channels).sum():
            ...
            new_shell = list(
                sorted(c for c in new_shell if (c not in already_seen) and (c in neighbors))
            )
