...
class AlignSnippets(BaseSnippets):
    ...
    def __init__(
        self, snippets, new_nbefore, new_nafter, mode="main_peak", interpolate=1, det_sign=0
    ):
...
class AlignSnippetsSegment(BaseSnippetsSegment):
    ...
    def get_snippets(
        ...
        if self._interpolate > 1:
            ...
            snippets = CubicSpline(xs[:: self._interpolate], snippets, axis=1, bc_type="natural")(
                xs
            )
