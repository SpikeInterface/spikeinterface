    ...
    if total_length < bin_duration_samples:
        ...
        for unit_id in unit_ids:
            ...
            presence_ratios[unit_id] = presence_ratio(
                spike_train, total_length, bin_edges=bin_edges
            )

    return presence_ratios


_default_params["presence_ratio"] = dict(bin_duration_s=60)
...
def compute_amplitude_cutoffs(
    ...
    for unit_id in unit_ids:
        if spike_amplitudes is None:
            ...
            if waveform_extractor.is_sparse():
                chan_ind = np.where(
                    waveform_extractor.sparsity.unit_id_to_channel_ids[unit_id] == chan_id
                )[0]
            else:
    ...
    if len(nan_units) > 0:
        warnings.warn(
            f"Units {nan_units} have too few spikes and " "amplitude_cutoff is set to NaN"
        )
...
def amplitude_cutoff(
    amplitudes, num_histogram_bins=500, histogram_smoothing_value=3, amplitudes_bins_min_ratio=5
):
    """Calculate approximate fraction of spikes missing from a distribution of amplitudes.
...
if HAVE_NUMBA:
    ...
    @numba.jit(
        (numba.int32[::1], numba.int64[::1], numba.int32[::1], numba.int32, numba.int32),
        nopython=True,
        nogil=True,
        cache=True,
        parallel=True,
