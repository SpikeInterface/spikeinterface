...
# TODO: merge with amplitude cutoff?
def compute_noise_cutoff(waveform_extractor, peak_sign='neg', 
                         quartile_length=.2, num_histogram_bins=100, num_low_bins=2,
                         amplitudes_bins_min_ratio=5):
    """Calculate noise_cutoff, a metric describing whether an amplitude distribution is cut off, 
    similar to amplitude cutoff but without a Gaussian assumption.
    A histogram of amplitudes is created and quantifies the distance between the low tail, 
    mean number of spikes and high tail in terms of standard deviations.
    ...
    num_histogram_bins : int
        The number of bins used to compute a histogram of the amplitude distribution
    num_low_bins : int
        The number of bins used in the lower part of the distribution (where cutoff is determined)
    amplitudes_bins_min_ratio : int, optional, default: 5
        The minimum ratio between number of amplitudes for a unit and the number of bins.
        If the ratio is less than this threshold, the amplitude_cutoff for the unit is set 
        to NaN
    ...
    Reference
    ---------
    Ported from IBL implementation:
    https://github.com/int-brain-lab/ibllib/blob/2e1f91c622ba8dbd04fc53946c185c99451ce5d6/brainbox/metrics/single_units.py
    """
    recording = waveform_extractor.recording
    sorting = waveform_extractor.sorting
    unit_ids = sorting.unit_ids
    ...
    for unit_id in unit_ids:
        ...
        if len(amplitudes) / num_histogram_bins < amplitudes_bins_min_ratio:
            nan_units.append(unit_id)
            noise_cutoff[unit_id] = np.nan
            continue

        # bins_list = np.linspace(0, np.max(amps), num_histogram_bins)
        # n, bins = np.histogram(amps, bins=bins_list)
        n, bins = np.histogram(amps, bins=num_histogram_bins)
        dx = np.diff(n)
        idx_nz = np.nonzero(dx)  # indices of nonzeros
        idx_peak = np.argmax(n)
        print(idx_peak, idx_nz)
        ...
        print(high_quartile_start_ind)
        xx = idx_nz[0][idx_nz[0] > high_quartile_start_ind]
        print(len(n[xx]))
        if len(n[xx]) > 0:
            mean_high_quartile = np.mean(n[xx])
            std_high_quartile = np.std(n[xx])
            first_low_quartile = np.mean(n[idx_nz[0][1:num_low_bins]])
            print(std_high_quartile)
