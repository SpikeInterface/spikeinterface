...
class KilosortSorter(KilosortBase, BaseSorter):
    ...
    def set_kilosort_path(cls, kilosort_path: str):
        ...
        try:
            print(
                "Setting KILOSORT_PATH environment variable for subprocess calls to:", kilosort_path
            )
    ...
    def _get_specific_options(cls, ops, params):
        ...
        ops["Th"] = [
            ...
            10.0,
            10.0,
        ]  # threshold for detecting spikes on template-filtered data ([6 12 12])
        ops["lam"] = [
            5.0,
            5.0,
            5.0,
        ]  # large means amplitudes are forced around the mean ([10 30 30])
        ...
        ops["loc_range"] = [
            3.0,
            1.0,
        ]  # ranges to detect peaks; plus/minus in time and channel ([3 1])
