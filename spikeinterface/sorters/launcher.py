...
from spikeinterface.core import aggregate_units, load_extractor
from spikeinterface.core.core_tools import check_json

from .runsorter import _common_param_doc, run_sorter
...
def run_sorters(
    ...
    if isinstance(recording_dict_or_list, list):
        # in case of list
        recording_dict = {
            "recording_{}".format(i): rec for i, rec in enumerate(recording_dict_or_list)
        }
...
def _check_container_images(docker_image, singularity_image, sorter_name):
    if docker_image is not None:
        assert singularity_image is None, (
            f"Provide either a docker or a singularity image " f"for sorter {sorter_name}"
        )
