...
def test_run_sorters_with_list():
    ...
    run_sorters(
        sorter_list, recording_list, working_folder, engine="loop", verbose=False, with_output=False
    )
...
def test_run_sorters_with_dict():
    ...
    sorter_params = {"tridesclous": dict(detect_threshold=5.6), "tridesclous2": dict()}
