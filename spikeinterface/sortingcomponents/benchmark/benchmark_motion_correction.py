...
class BenchmarkMotionCorrectionMearec(BenchmarkBase):
    ...
    def __init__(
        ...
        self._args.extend(
            [str(mearec_filename_drifting), str(mearec_filename_static), None, None, None]
        )
    ...
    def extract_waveforms(self):
        ...
        for key in self.keys:
            ...
            we.set_params(
                ms_before=2.0, ms_after=3.0, max_spikes_per_unit=500.0, return_scaled=True
            )
    ...
    def run_sorters(self):
        for case in self.sorter_cases:
            ...
            sorting = run_sorter(
                sorter_name, recording, output_folder, **sorter_params, delete_output_folder=True
            )
    ...
    def compute_distances_to_static(self, force=False):
        ...
        for key in self.keys:
            ...
            for unit_ind, unit_id in enumerate(self.waveforms[key].sorting.unit_ids):
                ...
                dist["template_euclidean"][unit_ind] = sklearn.metrics.pairwise_distances(
...
def plot_distances_to_static(benchmarks, metric="cosine", figsize=(15, 10)):
    ...
    for count, bench in enumerate(benchmarks):
        ...
        m_differences = (
            distances["corrected"][f"wf_{metric}_mean"] / distances["static"][f"wf_{metric}_mean"]
        )
        s_differences = (
            distances["corrected"][f"wf_{metric}_std"] / distances["static"][f"wf_{metric}_std"]
        )
...
def plot_snr_decrease(benchmarks, figsize=(15, 10)):
    ...
    axes[0, 1].plot(
        distances_to_center[idx], (snr_drifting.values / snr_static.values)[idx], c="0.5"
    )
    ...
    for count, bench in enumerate(benchmarks):
        ...
        axes[1, 1].plot(
            distances_to_center[idx], (snr_corrected.values / snr_static.values)[idx], c=f"C{count}"
        )
...
def plot_residuals_comparisons(benchmarks):
    ...
    for count, bench in enumerate(benchmarks):
        ...
        axes[1].plot(
            ...
            residuals["corrected"].mean(1)[idx],
            label=bench.title,
            lw=2,
            c=f"C{count}",
        )
    ...
    for count, bench in enumerate(benchmarks):
        ...
        axes[2].bar(
            [count],
            [residuals["corrected"].mean()],
            yerr=[residuals["corrected"].std()],
            color=f"C{count}",
        )
