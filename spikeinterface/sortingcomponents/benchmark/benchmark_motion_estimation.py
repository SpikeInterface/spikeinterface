...
class BenchmarkMotionEstimationMearec(BenchmarkBase):
        ...
        if self.spatial_bins is None:
            ...
            for t in range(self.gt_unit_positions.shape[0]):
                f = scipy.interpolate.interp1d(
                    unit_positions, unit_motions[t, :], fill_value="extrapolate"
                )
    ...
    def plot_peaks_probe(self, alpha=0.05, figsize=(15, 10)):
        ...
        if "z" in self.peak_locations.dtype.fields:
            ax = axs[1]
            ax.scatter(
                self.peak_locations["z"], self.peak_locations["y"], color="k", s=1, alpha=alpha
            )
            ax.set_xlabel("z")
            ax.set_xlim(0, 100)

    def plot_peaks(
        self, scaling_probe=1.5, show_drift=True, show_histogram=True, alpha=0.05, figsize=(15, 10)
    ):
        fig = plt.figure(figsize=figsize)
        if show_histogram:
    ...
    def plot_motion_corrected_peaks(
        self, scaling_probe=1.5, alpha=0.05, figsize=(15, 10), show_probe=True, axes=None
    ):
        if axes is None:
            fig = plt.figure(figsize=figsize)
            if show_probe:
    ...
    def estimation_vs_depth(self, show_only=8, figsize=(15, 10)):
        ...
        for i in range(0, n, step):
            ...
            ax.plot(
                self.temporal_bins,
                self.motion[:, i] - self.gt_motion[:, i],
                lw=1.5,
                ls="-",
                color=colors(i),
            )
...
def plot_motions_several_benchmarks(benchmarks):
    ...
    ax.plot(
        list(benchmarks)[0].temporal_bins,
        list(benchmarks)[0].gt_motion[:, 0],
        lw=2,
        c="k",
        label="real motion",
    )
