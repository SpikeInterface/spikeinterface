...
from spikeinterface.core import NumpySorting, extract_waveforms, get_noise_levels
...
class BenchmarkPeakSelection:
    ...
    def plot_clusters_amplitudes(
        ...
        for unit_id in self.gt_sorting.unit_ids:
            ...
            ax.scatter(
                self.gt_positions["x"][mask], self.gt_positions["y"][mask], c=colors, s=1, alpha=0.5
            )
            x_mean, y_mean = (
    ...
    def plot_statistics(self, metric="cosine", annotations=True, detect_threshold=5):
        ...
        garbage_snrs = (
            self.garbage_peaks["amplitude"] / noise_levels[self.garbage_peaks["channel_ind"]]
        )
        ...
        ax.hist(
            self.sliced_gt_peaks["amplitude"] / noise_levels[self.sliced_gt_peaks["channel_ind"]],
        ...
        for found, real in zip(unit_ids2, inds_1):
            mask = labels == found
            center = np.array(
                [self.sliced_gt_positions[mask]["x"], self.sliced_gt_positions[mask]["y"]]
            ).mean()
