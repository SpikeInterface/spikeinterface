    ...
    def _find_clusters(cls, recording, peaks, wfs_arrays, sparsity_mask, noise, d):
        ...
        while True:
            ...
            clusterer = hdbscan.HDBSCAN(
                min_cluster_size=d["min_cluster_size"], allow_single_cluster=True, metric="l2"
            )
            ...
            if num_cluster > 1:
                ...
                (outlier_inds,) = np.nonzero(
                    (local_labels == -1) & (peaks[local_peak_ind]["channel_ind"] == local_chan_ind)
                )
            ...
            if plot_debug:
                ...
                for label in local_labels_set:
                    ...
                    mask = local_labels == label
                    ax.scatter(
                        reduce_local_feature[mask, 0], reduce_local_feature[mask, 1], color=color
                    )
