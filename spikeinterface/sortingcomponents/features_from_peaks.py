...
def compute_features_from_peaks(
    ...
    **job_kwargs,
    ...
    extract_dense_waveforms = ExtractDenseWaveforms(
        recording, ms_before=ms_before, ms_after=ms_after, return_ouput=False
    )
    nodes = [
    ...
    features = run_peak_pipeline(
        recording, peaks, nodes, job_kwargs, job_name="features_from_peaks", squeeze_output=False
    )
...
class AmplitudeFeature(PipelineNode):
    def __init__(
        ...
        PipelineNode.__init__(
            self, recording, name=name, return_ouput=return_ouput, parents=parents
        )
...
class RandomProjectionsFeature(PipelineNode):
    def __init__(
        ...
        self._kwargs.update(
            dict(projections=projections, local_radius_um=local_radius_um, min_values=min_values)
        )
...
class RandomProjectionsEnergyFeature(PipelineNode):
    def __init__(
        ...
        self.local_radius_um = local_radius_um
        self._kwargs.update(
            dict(projections=projections, min_values=min_values, local_radius_um=local_radius_um)
        )
        self._dtype = recording.get_dtype()
    ...
    def compute(self, traces, peaks, waveforms):
        ...
        for main_chan in np.unique(peaks["channel_ind"]):
            ...
            all_projections[idx[mask]] = np.dot(energies[mask], local_projections) / (
                denom[mask][:, np.newaxis]
            )
        return all_projections
