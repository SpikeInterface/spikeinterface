...
def _detect_peaks_chunk(segment_index, start_frame, end_frame, worker_ctx):
    ...
    if method == 'by_channel':
        peak_sample_ind, peak_chan_ind = detect_peaks_by_channel(\
            trace_detection, peak_sign, abs_threholds, n_shifts)
    ...
    if localization_dict is not None:
        ...
        if localization_dict['method'] == 'center_of_mass':
            peak_locations = localize_peaks_center_of_mass(\
                traces, peaks, contact_locations, neighbours_mask_for_loc, nbefore, nafter)
            ...
            peak_locations = localize_peaks_monopolar_triangulation(\
                traces, peaks, contact_locations, neighbours_mask_for_loc, nbefore, nafter, max_distance_um)
