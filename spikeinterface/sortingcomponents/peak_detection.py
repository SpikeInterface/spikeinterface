...
from spikeinterface.core.recording_tools import get_channel_distances, get_noise_levels
    ...
    def detect_peaks(cls, traces, peak_sign, abs_threholds, exclude_sweep_size):
        ...
        if peak_sign in ("pos", "both"):
            ...
            for i in range(exclude_sweep_size):
                ...
                peak_mask &= (
                    traces_center
                    >= traces[exclude_sweep_size + i + 1 : exclude_sweep_size + i + 1 + length, :]
                )
                    ...
                    for i in range(exclude_sweep_size):
                        if chan_ind != neighbour:
                            peak_mask[s, chan_ind] &= (
                                traces_center[s, chan_ind] >= traces_center[s, neighbour]
                            )
                        peak_mask[s, chan_ind] &= (
