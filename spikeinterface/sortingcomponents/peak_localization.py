...
from .peak_pipeline import ExtractDenseWaveforms, PipelineNode, run_peak_pipeline
...
def localize_peaks(
    ...
    peak_locations = run_peak_pipeline(
        recording, peaks, pipeline_nodes, job_kwargs, job_name="localize peaks", squeeze_output=True
    )
...
class LocalizeBase(PipelineNode):
    def __init__(self, recording, return_ouput=True, parents=None, local_radius_um=75.0):
...
class LocalizeCenterOfMass(LocalizeBase):
    ...
    def __init__(
        self, recording, return_ouput=True, parents=["extract_waveforms"], local_radius_um=75.0
    ):
    ...
    def compute(self, traces, peaks, waveforms):
        ...
        for i, peak in enumerate(peaks):
            ...
            peak_locations[i] = solve_monopolar_triangulation(
                wf_ptp, local_contact_locations, self.max_distance_um, self.optimizer
            )
