...
from spikeinterface.core import get_channel_distances, get_chunk_with_margin
...
class ExtractDenseWaveforms(PipelineNode):
    def __init__(self, recording, return_ouput=False, ms_before=None, ms_after=None):
...
class ExtractSparseWaveforms(PipelineNode):
    ...
    def compute(self, traces, peaks):
        sparse_wfs = np.zeros(
            (peaks.shape[0], self.nbefore + self.nafter, self.max_num_chans), dtype=traces.dtype
        )
...
def run_peak_pipeline(
    recording, peaks, nodes, job_kwargs, job_name="peak_pipeline", squeeze_output=True
):
    """
    ...
    processor = ChunkRecordingExecutor(
        recording,
        _compute_peak_step_chunk,
        _init_worker_peak_pipeline,
        init_args,
        handle_returns=True,
        job_name=job_name,
        **job_kwargs,
...
def run_nodes(traces_chunk, local_peaks, nodes):
    ...
    for node in nodes:
        ...
        for parent in node_parents:
            ...
            parent_outputs_tuple = (
                parent_output if isinstance(parent_output, tuple) else (parent_output,)
            )
