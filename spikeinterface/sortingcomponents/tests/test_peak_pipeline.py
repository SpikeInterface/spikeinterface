...
from spikeinterface import BaseSorting, download_dataset
...
def test_run_peak_pipeline():
    ...
    waveforms_rms = WaveformsRootMeanSquare(
        recording, parents=[extract_waveforms], return_ouput=True
    )
    denoised_waveforms_rms = WaveformsRootMeanSquare(
        recording, parents=[waveform_denoiser], return_ouput=True
    )
