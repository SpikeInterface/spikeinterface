...
def test_pca_projection(tmp_path):
    ...
    temporal_pca.fit(recording, n_components=n_components, ms_before=ms_before, ms_after=ms_after, 
                     detect_peaks_params=detect_peaks_params, peak_selection_params=peak_selection_params, 
...
def test_pca_projection_sparsity(tmp_path):
    ...
    extract_waveforms = ExtractSparseWaveforms(recording=recording, ms_before=ms_before,
                                              ms_after=ms_after, local_radius_um=local_radius_um, return_ouput=True)
    temporal_pca = TemporalPCAProjection(recording=recording, model_path=model_path, parents=[extract_waveforms])
    pipeline_nodes = [extract_waveforms, temporal_pca]   
