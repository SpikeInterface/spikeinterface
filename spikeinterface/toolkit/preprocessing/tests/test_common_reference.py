...
def test_common_reference():
    ...
    rec_local_car = common_reference(rec, reference='local', local_radius=(20, 65))
    ...
    print(traces[:, 0].shape, rec_local_car.get_traces()[:, 0].shape)
    print(np.mean(traces[:, [2, 3]], axis=1, keepdims=True).shape)
