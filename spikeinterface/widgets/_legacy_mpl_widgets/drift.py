            ...
            for i in range(nchunk):
                ...
                if self.weight_with_amplitudes:
                    count, bins = np.histogram(
                        depths, bins=depth_bins, weights=np.abs(peaks["amplitude"][mask])
                    )
                else:
...
class PairwiseDisplacementWidget(BaseWidget):
    ...
    def __init__(
        self, motion, temporal_bins, spatial_bins, extra_check, figure=None, ax=None, ncols=5
    ):
...
class DisplacementWidget(BaseWidget):
    ...
    def _do_plot(self):
        ...
        if self.with_histogram:
            ...
            extent = (
                temporal_hist_bins[0],
                temporal_hist_bins[-1],
                spatial_hist_bins[0],
                spatial_hist_bins[-1],
            )
            ax.imshow(
