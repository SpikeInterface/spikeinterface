...
class StudyComparisonPerformancesByTemplateSimilarity(BaseWidget):
    ...
    def plot(self):
        ...
        for rec_name in self.study.rec_names:
            waveform_folder = (
                self.study.study_folder / "waveforms" / f"waveforms_GroundTruth_{rec_name}"
            )
        ...
        similarity_means = [
            all_results[sorter_name]["similarity"].mean() for sorter_name in self.study.sorter_names
        ]
        similarity_stds = [
            all_results[sorter_name]["similarity"].std() for sorter_name in self.study.sorter_names
        ]

        accuracy_means = [
            all_results[sorter_name]["accuracy"].mean() for sorter_name in self.study.sorter_names
        ]
        accuracy_stds = [
            all_results[sorter_name]["accuracy"].std() for sorter_name in self.study.sorter_names
        ]
