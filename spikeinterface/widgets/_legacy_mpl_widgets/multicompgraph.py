...
class MultiCompGlobalAgreementWidget(BaseWidget):
    ...
    def __init__(
        self, multi_comparison, plot_type="pie", cmap="YlOrRd", fs=10, figure=None, ax=None
    ):
        BaseWidget.__init__(self, figure, ax)
        self._msc = multi_comparison
    ...
    def _do_plot(self):
        ...
        if self._type == "pie":
            p = self.ax.pie(
                c, colors=colors[v - 1], autopct=lambda pct: _getabs(pct, c), pctdistance=1.25
            )
            self.ax.legend(
                p[0],
                v,
...
class MultiCompAgreementBySorterWidget(BaseWidget):
    ...
    def __init__(
        self, multi_comparison, plot_type="pie", cmap="YlOrRd", fs=9, axes=None, show_legend=True
    ):
        self._msc = multi_comparison
        self._type = plot_type
        self._cmap = cmap
    ...
    def _do_plot(self):
        ...
        for i, name in enumerate(name_list):
            ax = self.axes[i]
            v, c = np.unique(
                [len(np.unique(sn)) for sn in sg_names if name in sn], return_counts=True
            )
            if self._type == "pie":
                p = ax.pie(
                    c,
