...
class TestWidgets(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.backend_kwargs = {"matplotlib": {}, "sortingview": {}, "ipywidgets": {}}
        ...
        for backend in possible_backends:
            ...
            if backend not in self.skip_backends:
                ...
                if backend != "sortingview":
                    sw.plot_timeseries(
                        self.recording, mode="auto", backend=backend, **self.backend_kwargs[backend]
                    )
                    sw.plot_timeseries(
                        self.recording,
                        mode="line",
        ...
        for backend in possible_backends:
            if backend not in self.skip_backends:
                sw.plot_unit_locations(
                    self.we, with_channel_ids=True, backend=backend, **self.backend_kwargs[backend]
                )
                sw.plot_unit_locations(
                    self.we_sparse,
