...
def setup_module():
    ...
    wvf_extractor = extract_waveforms(
        recording,
        sorting,
        folder=cache_folder / "wvf_extractor",
        ms_before=10.0,
        ms_after=10.0,
    )
...
def test_hybrid_units_recording():
    ...
    assert hybrid_units_recording.get_traces(end_frame=600, segment_index=0).shape == (
        600,
        4,
    )
...
def test_hybrid_spikes_recording():
    ...
    hybrid_spikes_recording = create_hybrid_spikes_recording(
        wvf_extractor,
        unit_ids=sorting.unit_ids[:3],
        injected_sorting_folder=cache_folder / "injected2",
    )

    assert hybrid_spikes_recording.get_traces(end_frame=600, segment_index=0).shape == (
        600,
        4,
    )
