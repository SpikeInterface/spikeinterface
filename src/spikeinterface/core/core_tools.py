...
def anononymous_zarr_open(folder_path: str | Path, mode: str = "r", storage_options: dict | None = None):
    """
    Open a zarr folder. If  with anonymous access.
    ...
    # if storage_options is None, we try to open the folder without and with anonymous access
    # if storage_options is not None, we try to open the folder with the given storage options
    if storage_options is None:
        storage_options_to_test = ({"anon": False}, {"anon": True})
    else:
