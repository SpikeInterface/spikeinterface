...
def synthesize_poisson_spike_vector(
    ...
    Notes
    -----
    - The function uses a geometric distribution to simulate the discrete inter-spike intervals (in frames).
    This is the discrete equivalent of an exponential distribution which is the distribution of
    the inter-spike intervals (in times) when the spikes are generated by a Poisson process.
    - The refractory period is implemented by adding a fixed number of frames to the inter-spike intervals.
    - The function uses the model described in [1]_ where the firing rate is modified to account for the
    refractory period. That is, the firing rate is increased outside of the refactory period so as to keep
    the original firing rate when averaging over the whole time period.

    More precisely, the firing rate is given by :math:`\\lambda' = \\frac{\\lambda}{1 - \\lambda \\tau}`
    where :math:`\\lambda` is the original firing rate and :math:`\\tau` is the refractory period.


    [1] Moritz Deger, Moritz Helias, Clemens Boucsein, and Stefan Rotter.
    Statistical properties of superimposed stationary spike trains. Journal of Computational Neuroscience, 32(3):443–463, 2012.
    """
    ...
    binomial_p_max = np.max(binomial_p).item()
    max_frames = duration * sampling_frequency
    num_spikes_expected = ceil(max_frames * binomial_p_max)
    num_spikes_std = int(np.sqrt(num_spikes_expected * (1 - binomial_p_max)))
