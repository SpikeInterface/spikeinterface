...
def test_BaseSnippets():
    ...
    snippets = NpySnippetsExtractor(file_paths, sampling_frequency=nse.get_sampling_frequency(),
                channel_ids=nse.channel_ids, nbefore=nse.nbefore,
                gain_to_uV=nse.get_property('gain_to_uV'), offset_to_uV=nse.get_property('offset_to_uV')
                )
    ...
    assert np.all(snippets.ids_to_indices(
    ...
    snippets.set_property('quality', [1., 3.3, np.nan])
    ...
    assert_raises(Exception, snippets.set_property, key='string_property_nan', values=["hola", "chabon"], ids=[0, 1],
                  missing_value=np.nan)

    # int properties without missing values raise an error
    assert_raises(Exception, snippets.set_property,
    ...
    nse_int16 = NumpySnippets(snippets_list=snippets_list, spikesframes_list=spikesframes_list,
                        sampling_frequency = 30000)
    ...
    nse_int16.set_property('gain_to_uV', [.195] * 5)
    nse_int16.set_property('offset_to_uV', [0.] * 5)
    waveforms_float32 = nse_int16.get_snippets(return_scaled=True)
