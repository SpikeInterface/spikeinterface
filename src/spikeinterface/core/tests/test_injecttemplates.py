...
def test_inject_templates():
    ...
    wvf_extractor = extract_waveforms(
        recording, sorting, mode="memory", ms_before=3.0, ms_after=3.0
    )
    templates = wvf_extractor.get_all_templates()
    templates[:, 0] = templates[
        :, -1
    ] = 0.0  # Go around the check for the edge, this is just testing.
    ...
    recording_template_injected = InjectTemplatesRecording(
        ...
        num_samples=[
            recording.get_num_frames(seg_ind)
            for seg_ind in range(recording.get_num_segments())
        ],
    )

    assert recording_template_injected.get_traces(
        end_frame=600, segment_index=0
    ).shape == (600, 4)
    assert recording_template_injected.get_traces(
    ...
    check_recordings_equal(
        recording_template_injected, saved_loaded, return_scaled=False
    )

    saved_1job = recording_template_injected.save(folder=cache_folder / "1job")
    saved_2job = recording_template_injected.save(
        folder=cache_folder / "2job", n_jobs=2, chunk_duration="1s"
    )
