...
def test_encoding_dict_with_array_values(
    numpy_array_integer, numpy_array_float, numpy_array_bool
):
    dict_with_array_as_values = dict(
        int=numpy_array_integer, float=numpy_array_float, boolean=numpy_array_bool
    )
    json.dumps(dict_with_array_as_values, cls=SIJsonEncoder)


def test_encoding_dict_with_array_values_nested(
    numpy_array_integer, numpy_array_float, numpy_array_bool
):
...
def test_encoding_numpy_scalars_values(
    numpy_array_integer, numpy_array_float, numpy_array_bool
):
...
def test_encoding_numpy_scalars_keys(
    numpy_array_integer, numpy_array_float, numpy_array_bool
):
    numpy_integer_scalar = numpy_array_integer[0]
    numpy_float_scalar = numpy_array_float[0]
...
def test_encoding_numpy_scalars_keys_nested(
    numpy_array_integer, numpy_array_float, numpy_array_bool
):
    numpy_integer_scalar = numpy_array_integer[0]
    numpy_float_scalar = numpy_array_float[0]
    ...
    dict_with_nested_numpy_scalars = {
        numpy_integer_scalar: {numpy_float_scalar: {numpy_boolean_scalar: "deep_value"}}
    }
    json.dumps(dict_with_nested_numpy_scalars, cls=SIJsonEncoder)


def test_encoding_numpy_scalars_keys_nestes_mixed(
    numpy_array_integer, numpy_array_float, numpy_array_bool
):
    numpy_integer_scalar = numpy_array_integer[0]
    numpy_float_scalar = numpy_array_float[0]
...
def test_numpy_dtype_alises_encoding():
    ...
    json.dumps(
        np.bool_, cls=SIJsonEncoder
    )  # Note that np.bool was deperecated in numpy 1.20.0
