...
def test_skip_noise_levels_propagation():
    ...
    rec_channel_slice = rec.channel_slice(channel_ids=rec.channel_ids[:2])
    rec_frame_slice = rec.frame_slice(start_frame=0, end_frame=1000)
    assert "noise_level_raw" in rec_channel_slice.get_property_keys()
    ...
    assert "noise_level_raw" in rec_channel_slice.get_property_keys()
    assert "noise_level_scaled" in rec_channel_slice.get_property_keys()
