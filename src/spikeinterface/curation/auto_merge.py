...
from ..core import start_sorting_result
...
def get_potential_auto_merge(
    ...
    if "remove_contaminated" in steps:
        contaminations, nb_violations = compute_refrac_period_violations(
            sorting_result, refractory_period_ms=refractory_period_ms, censored_period_ms=censored_period_ms
        )
    ...
    if "template_similarity" in steps:
        templates = sorting_result.get_extension("templates").get_templates(operator="average")
...
def check_improve_contaminations_score(
    ...
    firing_rates = list(compute_firing_rates(sorting_result).values())
    ...
    for i in range(inds1.size):
        ...
        sorting_result_new = start_sorting_result(sorting_merged, recording, format="memory", sparse=False)

        new_contaminations, _ = compute_refrac_period_violations(
            sorting_result_new, refractory_period_ms=refractory_period_ms, censored_period_ms=censored_period_ms
        )
        c_new = new_contaminations[unit_id1]
        f_new = compute_firing_rates(sorting_result_new)[unit_id1]
