...
def bombcell_label_units(
    ...
    label_non_somatic=None,
    split_non_somatic_good_mua=None,
) -> "pd.DataFrame":
    """
    ...
    if label_non_somatic is not None or split_non_somatic_good_mua is not None:
        warnings.warn(
            "Both 'label_non_somatic' and 'split_non_somatic_good_mua' are deprecated. "
    ...
    if len(missing_columns) > 0:
        warnings.warn(
            f"Bombcell thresholds reference metrics not found in the available metrics and will be skipped. "
            f"Compute them first if you want them included in the labeling. Missing columns: {missing_columns}"
        )
