...
from spikeinterface import SortingResult
...
def remove_redundant_units(
    ...
    Parameters
    ----------
    sorting_or_sorting_result : BaseSorting or SortingResult
        If SortingResult, the spike trains can be optionally realigned using the peak shift in the
        template to improve the matching procedure.
        If BaseSorting, the spike trains are not aligned.
    align : bool, default: False
        If True, spike trains are aligned (if a SortingResult is used)
    ...
    if isinstance(sorting_or_sorting_result, SortingResult):
        sorting = sorting_or_sorting_result.sorting
        sorting_result = sorting_or_sorting_result
    else:
        assert not align, "The 'align' option is only available when a SortingResult is used as input"
        sorting = sorting_or_sorting_result
        sorting_result = None

    if align and unit_peak_shifts is None:
        assert sorting_result is not None, "For align=True must give a SortingResult or explicit unit_peak_shifts"
        unit_peak_shifts = get_template_extremum_channel_peak_shift(sorting_result)
    ...
    if remove_strategy in ("minimum_shift", "highest_amplitude"):
        ...
        peak_values = get_template_amplitudes(sorting_result, peak_sign=peak_sign, mode="at_index")
    ...
    if remove_strategy == "minimum_shift":
        ...
        # TODO
        # @aurelien @alessio
        # here sorting_result can implement the choice of the best one given an external metrics table
