...
def test_remove_excess_spikes():
    ...
    recording = generate_recording(
        durations=durations, sampling_frequency=sampling_frequency
    )
    ...
    for segment_index in range(recording.get_num_segments()):
        ...
        for unit in range(num_units):
            ...
            last_samples_spikes = (num_samples - 1) * np.ones(
                num_num_samples_spikes_per_segment, dtype=int
            )
            num_samples_spike_times = num_samples * np.ones(
                num_num_samples_spikes_per_segment, dtype=int
            )
            excess_spikes = np.random.randint(
                num_samples, num_samples + 100, num_excess_spikes_per_segment
            )
            spike_times = np.sort(
                np.concatenate(
                    (
    ...
    sorting = NumpySorting.from_times_labels(
        times, labels, sampling_frequency=sampling_frequency
    )
    ...
    for u in sorting.unit_ids:
        for segment_index in range(sorting.get_num_segments()):
            spike_train_excess = sorting.get_unit_spike_train(
                u, segment_index=segment_index
            )
            spike_train_corrected = sorting_corrected.get_unit_spike_train(
                u, segment_index=segment_index
            )
