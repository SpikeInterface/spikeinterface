...
def export_to_ibl(
    ...
    **job_kwargs,
    ...
    print("Doing phy-like export part...")
    if os.path.exists(output_folder) and ~remove_if_exists:
        print("Output folder already exists. Skipping phy-like export.")
    else:
        export_to_phy(
            waveform_extractor,
            output_folder,
    ...
    print("Adding IBL specific steps...")

    # Now we need to add the extra IBL specific files
    channel_inds, = np.isin(recording.channel_ids, waveform_extractor.channel_ids).nonzero()
    ...
    if "spike_locations" not in available_extension_names:
        ...
        compute_spike_locations(waveform_extractor, verbose=verbose)  # this should auto-save it
    ...
    np.save(output_folder / "spike_clusters.npy",
            np.squeeze(clusters.astype('uint32')))
    
    # convert times and squeeze
    times = np.load(output_folder / "spike_times.npy")
    np.save(output_folder / "spike_times.npy", np.squeeze(times / 30000.).astype('float64'))
    ...
    extremum_channel_indices = get_template_extremum_channel(waveform_extractor, outputs="index")
    ...
    for unit_idx, unit_id in enumerate(waveform_extractor.unit_ids):
        ...
        peakToTrough = (np.argmax(peak_waveform) - np.argmin(peak_waveform)) / waveform_extractor.sampling_frequency
        # cluster_channels.append(int(channel_locs[extremum_channel_index, 1] / 10)) #???
        cluster_channels.append(extremum_channel_index)  # JP debug
