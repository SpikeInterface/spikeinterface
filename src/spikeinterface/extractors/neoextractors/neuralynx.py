...
class NeuralynxSortingExtractor(NeoBaseSortingExtractor):
    ...
    def __init__(self, folder_path: str, sampling_frequency: float | None = None):
        neo_kwargs = self.map_to_neo_kwargs(folder_path)
        NeoBaseSortingExtractor.__init__(self, sampling_frequency=sampling_frequency, **neo_kwargs)
        self._kwargs.update(dict(folder_path=str(folder_path), sampling_frequency=self._sampling_frequency))
