...
def test_AlignSnippets():
    ...
    alined_snippets = AlignSnippets(snippets, new_nbefore=10, new_nafter=44,
                                    mode='main_peak', interpolate=5, det_sign=0)
    spikes = alined_snippets.get_snippets(segment_index=0)

    # test interpolation
    snippets_ch_peak = AlignSnippets(snippets, new_nbefore=15, new_nafter=30,
                                     mode='ch_peak', interpolate=5, det_sign=-1)
    spikes = snippets_ch_peak.get_snippets(segment_index=0)

    # test interpolation
    snippets_interpolated = AlignSnippets(snippets, new_nbefore=15, new_nafter=20,
                                          mode='ch_peak', interpolate=5, det_sign=-1)
