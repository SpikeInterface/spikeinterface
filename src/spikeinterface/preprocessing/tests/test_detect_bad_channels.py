...
def test_remove_bad_channels_ibl(num_channels):
    ...
    # Detect in SI
    bad_channel_ids, bad_channel_labels_si = detect_bad_channels(recording, method="coherence+psd",
    ...
    channel_flags_ibl = np.zeros((recording.get_num_channels(),
                                  recording.get_num_segments() * 10),
                                  dtype=int)
    ...
    recording_scrambled = recording.channel_slice(np.random.choice(recording.channel_ids,
                                                  len(recording.channel_ids), replace=False))
    bad_channel_ids_scrambled, bad_channel_label_scrambled = \
        detect_bad_channels(recording_scrambled, method="coherence+psd",
                            psd_hf_threshold=psd_cutoff,
                            dead_channel_threshold=-0.5,
                            noisy_channel_threshold=1,
                            outside_channel_threshold=-0.75,
...
def reduce_high_freq_power_in_non_noisy_channels(recording, is_noisy, not_noisy):
    ...
    fscale, psd = scipy.signal.welch(data,
...
def add_dead_channels(recording, is_dead):
    ...
    for segment_index in range(recording.get_num_segments()):
        ...
        data[:, is_dead] = np.random.normal(mean, std * 0.1,
                                            size=(is_dead.size, recording.get_num_samples(segment_index))).T
