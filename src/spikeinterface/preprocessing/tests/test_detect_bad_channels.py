...
def test_remove_bad_channels_ibl(num_channels):
    ...
    assert np.array_equal(
        recording.ids_to_indices(bad_channel_ids),
        np.where(bad_channel_labels_ibl != 0)[0],
    )
...
def add_dead_channels(recording, is_dead):
    ...
    for segment_index in range(recording.get_num_segments()):
        ...
        data[:, is_dead] = np.random.normal(
            mean,
            std * 0.1,
            size=(is_dead.size, recording.get_num_samples(segment_index)),
        ).T
