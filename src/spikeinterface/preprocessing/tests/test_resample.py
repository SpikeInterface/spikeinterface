...
def create_sinusoidal_traces(sampling_frequency=3e4, duration=30, freqs_n=10, 
                             max_freq=10000, dtype=np.int16):
...
def test_resample_freq_domain():
    ...
    assert np.allclose([resamp_rec.get_num_frames()
                       for resamp_rec in resamp_recs], resamp_fss * duration), msg1
    # They all last 1 second
    msg2 = "The total duration gets distorted by resampling."
    assert np.allclose([resamp_rec.get_total_duration()
                       for resamp_rec in resamp_recs], duration), msg2
    ...
    assert np.all(
        [rec.get_traces().shape[0] == rec.get_times().shape[0]
         for rec in resamp_recs]
    ), msg4
    ...
    if DEBUG:
        ...
        _ = ax.plot(parent_rec.get_times(), parent_rec.get_traces(), color="k", alpha=0.8,
                    lw=1.5, label="original")
        
        for i, rec in enumerate(resamp_recs):
            rs_rate = int(rec.get_sampling_frequency())
            _ = ax.plot(rec.get_times(), rec.get_traces(), color=f"C{i}", alpha=0.8,
                        lw=0.5, label=f"rate: {rs_rate}")
