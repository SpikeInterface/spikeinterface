...
def nearest_neighbors_isolation(
    ...
    if n_spikes_all_units[this_unit_id] < min_spikes:
        warnings.warn(
            f"Warning: unit {this_unit_id} has fewer spikes than specified by `min_spikes` ({min_spikes}); returning NaN as the quality metric..."
        )
        return np.nan, np.nan
    elif fr_all_units[this_unit_id] < min_fr:
        warnings.warn(
            f"Warning: unit {this_uit_id} has a firing rate below the specified `min_fr` ({min_fr}Hz); returning NaN as the quality metric..."
        )
        return np.nan, np.nan
    else:
...
def nearest_neighbors_noise_overlap(
    ...
    if n_spikes_all_units[this_unit_id] < min_spikes:
        ...
        warnings.warn(
            f"Warning: unit {this_unit_id} has a firing rate below the specified `min_fr` ({min_fr}Hz); returning NaN as the quality metric...",
        )
        return np.nan
    else:
