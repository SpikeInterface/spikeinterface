...
def check_if_installed(kilosort2_path: Union[str, None]):
    ...
    if (Path(kilosort2_path) / "master_kilosort.m").is_file() or (
        Path(kilosort2_path) / "main_kilosort.m"
    ).is_file():
        return True
    else:
        return False
...
class Kilosort2Sorter(KilosortBase, BaseSorter):
    ...
    def set_kilosort2_path(cls, kilosort2_path: PathType):
        ...
        try:
            print(
                "Setting KILOSORT2_PATH environment variable for subprocess calls to:",
                kilosort2_path,
            )
