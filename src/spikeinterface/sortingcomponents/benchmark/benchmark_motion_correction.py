...
class BenchmarkMotionCorrectionMearec(BenchmarkBase):
    ...
    def __init__(
        ...
        self._args.extend(
            [
                str(mearec_filename_drifting),
                str(mearec_filename_static),
                None,
                None,
                None,
            ]
...
def plot_distances_to_static(benchmarks, metric="cosine", figsize=(15, 10)):
    ...
    for count, bench in enumerate(benchmarks):
        ...
        ax_3.bar(
            [count],
            [m_differences.mean()],
            yerr=[m_differences.std()],
            color=f"C{count}",
        )
        ax_4.bar(
            [count],
            [s_differences.mean()],
            yerr=[s_differences.std()],
            color=f"C{count}",
        )
...
def plot_snr_decrease(benchmarks, figsize=(15, 10)):
    ...
    axes[0, 1].plot(
        ...
        (snr_drifting.values / snr_static.values)[idx],
        c="0.5",
    )
    ...
    for count, bench in enumerate(benchmarks):
        ...
        axes[1, 1].plot(
            distances_to_center[idx],
            (snr_corrected.values / snr_static.values)[idx],
            c=f"C{count}",
        )
