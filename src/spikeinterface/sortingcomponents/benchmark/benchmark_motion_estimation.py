...
class BenchmarkMotionEstimationMearec(BenchmarkBase):
    ...
    def compute_gt_motion(self):
        ...
        if self.spatial_bins is None:
            ...
            probe_y_min, probe_y_max = (
                channel_positions[:, 1].min(),
                channel_positions[:, 1].max(),
            )
    ...
    def plot_peaks_probe(self, alpha=0.05, figsize=(15, 10)):
        ...
        plot_probe_map(self.recording, ax=ax)
        ax.scatter(
            self.peak_locations["x"],
            self.peak_locations["y"],
            color="k",
            s=1,
            alpha=alpha,
        )
    ...
    def plot_peaks(
        ...
        if show_drift:
            if self.spatial_bins is None:
                ...
                ax.plot(
                    self.temporal_bins,
                    self.gt_motion[:, 0] + center,
                    color="green",
                    lw=1.5,
                )
                ax.plot(
                    self.temporal_bins,
                    self.motion[:, 0] + center,
                    color="orange",
                    lw=1.5,
                )
            else:
