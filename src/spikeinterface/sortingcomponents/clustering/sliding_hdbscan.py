        ...
        while True:
            ...
            for chan_ind in prev_local_chan_inds:
                if total_count[chan_ind] == 0:
                    continue
                # ~ inds, = np.nonzero(isin(peaks['channel_index'], closest_channels[chan_ind]) & (peak_labels==0))
            ...
            for chan_ind in local_chan_inds:
                ...
                # TODO: only for debug, remove later
                assert np.all(isin(local_chan_inds, wf_chans))

                # none label spikes
                wfs_chan = wfs_chan[inds, :, :]
                # only some channels
                wfs_chan = wfs_chan[:, :, isin(wf_chans, local_chan_inds)]
                wfs.append(wfs_chan)
...
def _collect_sparse_waveforms(peaks, wfs_arrays, closest_channels, peak_labels, sparsity_mask, label):
    ...
    for chan_ind in label_chan_inds:
        ...
        # TODO: only for debug, remove later
        assert np.all(isin(wanted_chans, wf_chans))
        wfs_chan = wfs_arrays[chan_ind]
        ...
        wfs_chan = wfs_chan[inds, :, :]
        # only some channels
        wfs_chan = wfs_chan[:, :, isin(wf_chans, wanted_chans)]
        wfs.append(wfs_chan)
