...
# TODO : handle DriftingTemplates sparse


class TridesclousDriftPeeler(BaseTemplateMatching):
    """
    Template-matching used by Tridesclous sorter.
    This is a modified version of tdc-peeler which move the template given a Motion.
    This avoid the Interpolation on tarces.
    """
    ...
    def __init__(
        ...
        if motion is None:
            raise ValueError("tdc-drift-peeler need a motion")
        self.motion = motion
    ...
    def compute_matching(self, traces, start_frame, end_frame, segment_index):
        ...
        for count, interp_bin_ind in enumerate(interpolation_bins_here):
            ...
            for spikes in spikes_in_time_bin:
                spikes["sample_index"] += current_start_index
