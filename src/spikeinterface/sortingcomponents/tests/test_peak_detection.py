...
def test_detect_peaks():
    ...
    if HAVE_TORCH:
        ...
        assert np.isclose(
            np.array(len(peaks_by_channel_np)),
            np.array(len(peaks_by_channel_torch)),
            rtol=0.1,
        )
    ...
    if HAVE_TORCH:
        ...
        # due to the different implementations, we allow a small tolerance
        assert np.isclose(
            np.array(len(peaks_local_numba)), np.array(len(peaks_local_torch)), rtol=0.1
        )
    ...
    extract_dense_waveforms = ExtractDenseWaveforms(
        recording, ms_before=1.0, ms_after=1.0, return_output=False
    )

    pipeline_nodes = [
        extract_dense_waveforms,
        PeakToPeakFeature(
            recording, all_channels=False, parents=[extract_dense_waveforms]
        ),
        LocalizeCenterOfMass(
            recording, local_radius_um=50.0, parents=[extract_dense_waveforms]
        ),
    ]
