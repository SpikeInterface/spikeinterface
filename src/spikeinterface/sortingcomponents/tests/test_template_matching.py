...
def waveform_extractor(folder):
    ...
    waveform_extractor = extract_waveforms(recording, gt_sorting, folder, overwrite=True,
...
def test_find_spikes_from_templates(method, waveform_extractor):
    ...
    if DEBUG:
        ...
        metrics = si.compute_quality_metrics(waveform_extractor, metric_names=['snr'], load_if_exists=True, )
