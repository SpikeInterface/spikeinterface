...
class SingleChannelToyDenoiser(PipelineNode):
    def __init__(
        ...
        try:
            waveform_extractor = next(
                parent
                for parent in self.parents
                if isinstance(parent, WaveformExtractorNode)
            )
        except (StopIteration, TypeError):
            exception_string = (
                f"Model should have a {WaveformExtractorNode.__name__} in its parents"
            )
    ...
    def load_model(self):
        ...
        denoiser = SingleChannel1dCNNDenoiser(
            pretrained_path=model_path, spike_size=128
        )
    ...
    def compute(self, traces, peaks, waveforms):
        ...
        # Denoise
        denoised_temporal_waveforms = (
            self.denoiser(temporal_waveforms_tensor).detach().numpy()
        )

        # Reconstruct representation with channels
        denoised_waveforms = from_temporal_representation(
            denoised_temporal_waveforms, num_channels
        )
...
class SingleChannel1dCNNDenoiser(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(1, out_channels_conv1, kernel_size_conv1), nn.ReLU()
        )
        self.conv2 = nn.Sequential(
            nn.Conv1d(out_channels_conv1, out_channels_conv_2, kernel_size_conv2),
            nn.ReLU(),
        )
        n_input_feat = out_channels_conv_2 * (
            spike_size - kernel_size_conv1 - kernel_size_conv2 + 2
        )
